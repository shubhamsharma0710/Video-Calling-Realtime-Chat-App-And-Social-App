...
const HomePage = () => {
  ...
  const { data: outgoingFriendReqsRaw = [] } = useQuery({
    queryKey: ["outgoingFriendReqs"],
    queryFn: getOutgoingFriendReqs,
  });
 
  const friends = friendsRaw.filter(Boolean);
  const recommendedUsers = usersRaw.filter(Boolean);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommendedUsers.map((user) => {
                const hasRequestBeenSent =
                  user && outgoingRequestsIds.has(user._id);
